...
async function getPaintings(): Promise<Product[]> {
  try {
    const response = await fetch("http://krinuh-be-ts.onrender.com/painting/all", {
      cache: "no-store",
    })

    if (!response.ok) {
      console.error("[v0] Failed to fetch paintings:", response.status)
      return []
    }
    ...
    if (!apiData.success || !apiData.data) {
      console.error("[v0] Invalid API response:", apiData)
      return []
    }
    ...
    console.error("[v0] Error fetching paintings:", error)
    return []
  }
}
